...
export default function Scheduler() {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <Card>
        <h2 className="mb-4 text-xl font-bold">Nuovo job [MODIFICATO]</h2>
        <div className="grid gap-3">
          ...
          {/* Inline Ambiguity UI */}
          {isNightTime && (
            <div className="bg-surface-2/50 border border-yellow-500/30 p-3 rounded mt-2">
              <p className="text-sm text-yellow-500 font-bold mb-2">⚠ Orario Notturno (00:00 - 05:00)</p>
              <div className="flex flex-col gap-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="ambiguity" checked={ambiguityChoice === 'same'} onChange={() => setAmbiguityChoice('same')} />
                  <span className="text-sm">Mattina Stesso Giorno (es. Venerdì Mattina)</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="ambiguity" checked={ambiguityChoice === 'next'} onChange={() => setAmbiguityChoice('next')} />
                  <span className="text-sm">Notte (Giorno Dopo) (es. Notte Ven-Sab)</span>
                </label>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={handleAddClick} className="px-6 py-2 rounded bg-primary text-black font-semibold hover:opacity-90 transition-opacity">
              Aggiungi
            </button>
          </div>
        </div>
      ...
      <Card>
        <h2 className="mb-4 text-xl font-bold">Eventi Recenti</h2>
        <div className="h-64 overflow-auto text-sm font-mono bg-surface-1 p-2 rounded">
          {events.length === 0 && <div className="text-gray-500 italic">Nessun evento recente</div>}
          {events.map((e, i) => (
            <div key={i} className="border-b border-white/10 py-1 opacity-80">
              <span className="text-xs text-gray-400">[{new Date().toLocaleTimeString()}]</span> {e.type} {e.job_id || ''} {e.message || ''}
            </div>
          ))}
        </div>
      </Card>
