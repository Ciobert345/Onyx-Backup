...
const Settings: React.FC = () => {
    ...
    const [minecraftPort, setMinecraftPort] = useState('25565');
    ...
    const loadSettings = async () => {
        ...
        if (ipcService.getPreferences) {
            ...
            if (prefs) {
                ...
                setMinecraftIp(prefs.minecraftIp || '');
                setMinecraftPort(prefs.minecraftPort || '25565');
            }
        }
        setLoading(false);
    ...
    }, [autoStart, showStats, minecraftIp, minecraftPort, loading]);
    ...
    return (
        <div className="space-y-10 animate-fade-in relative">
            ...
            <section>
                <h2 className="text-xl font-medium text-primary mb-6 mt-10">Integrations</h2>
                <div className="bg-surface border border-border rounded-xl p-6 sm:p-8 backdrop-blur-sm bg-opacity-80">
                    <div className="p-4 bg-surfaceHighlight rounded-lg border border-border">
                        <div className="mb-4">
                            <div className="text-primary font-medium">Minecraft Server Status</div>
                            <div className="text-sm text-secondary">Display server status and player count on dashboard</div>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                            <div className="sm:col-span-3">
                                <label className="block text-xs uppercase text-neutral-500 font-bold tracking-wider mb-2">Server Address</label>
                                <input
                                    type="text"
                                    value={minecraftIp}
                                    onChange={(e) => {
                                        // Sanitize input: remove http://, https://, and spaces
                                        const val = e.target.value.replace(/^https?:\/\//, '').replace(/\/$/, '').trim();
                                        setMinecraftIp(val);
                                    }}
                                    placeholder="e.g. mc.hypixel.net"
                                    className="w-full bg-[#0A0A0A] border border-neutral-700 rounded-lg px-4 py-2 text-primary focus:outline-none focus:border-neutral-500 transition-colors placeholder-neutral-700"
                                />
                            </div>
                            <div className="sm:col-span-1">
                                <label className="block text-xs uppercase text-neutral-500 font-bold tracking-wider mb-2">Port</label>
                                <input
                                    type="text"
                                    value={minecraftPort}
                                    onChange={(e) => setMinecraftPort(e.target.value)}
                                    placeholder="25565"
                                    className="w-full bg-[#0A0A0A] border border-neutral-700 rounded-lg px-4 py-2 text-primary focus:outline-none focus:border-neutral-500 transition-colors placeholder-neutral-700 text-center"
                                />
                            </div>
                        </div>
                    </div>
