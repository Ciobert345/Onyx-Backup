...
import { Power, Clock, Plus, Trash2, Calendar, Check, Pencil, X, Folder, Layers, MoreHorizontal } from 'lucide-react';
import { ScheduleItem, PowerAction, SchedulerGroup } from '../types.ts';
...
const Scheduler: React.FC = () => {
  ...
  const loadSchedule = async () => {
    const { scheduler, groups } = await ipcService.getSchedule();
    setSchedule(sortSchedule(scheduler));
    setGroups(groups || []);
    setLoading(false);
  };
  ...
  const createGroup = async () => {
    ...
    await ipcService.saveSchedulerGroups(updatedGroups);
    setNewGroupName('');
  };

  const toggleGroup = async (id: string) => {
    const groupToToggle = groups.find(g => g.id === id);
    if (!groupToToggle) return;

    const newEnabledState = !groupToToggle.enabled;
    const updatedGroups = groups.map(g => g.id === id ? { ...g, enabled: newEnabledState } : g);
    setGroups(updatedGroups);

    // Sync all tasks in the group with the new group state (both ON and OFF)
    const updatedSchedule = schedule.map(item =>
      item.groupId === id ? { ...item, enabled: newEnabledState } : item
    );
    setSchedule(updatedSchedule);

    // Save both groups and schedule to ensure persistence
    await ipcService.saveSchedule({
  ...
  const deleteGroup = async (id: string) => {
    ...
    // Clear group assignment from tasks
    const updatedSchedule = schedule.map(item => item.groupId === id ? { ...item, groupId: undefined } : item);
    setSchedule(updatedSchedule);

    await ipcService.saveSchedule({ scheduler: updatedSchedule, groups: updatedGroups });
  };
  ...
  const handleSaveItem = () => {
    ...
    setSchedule(updated);
    ipcService.saveSchedule({ scheduler: updated });
    ...
    <div className="space-y-8 max-w-6xl mx-auto h-full flex flex-col animate-fade-in pb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full items-start">
        ...
        <div className="lg:col-span-4 h-full flex flex-col gap-6">
          ...
          <div className="bg-surface border border-border rounded-xl backdrop-blur-sm bg-opacity-80 shadow-lg p-5">
            <div className="flex items-center gap-2 mb-4">
              <Folder size={18} className="text-primary" />
              <h3 className="text-base font-semibold text-primary">Task Groups</h3>
            </div>
            ...
            <div className="flex gap-2 mb-4">
              <input
                ...
                className="flex-1 bg-surfaceHighlight border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary"
                onKeyDown={(e) => e.key === 'Enter' && createGroup()}
              />
              <button
                onClick={createGroup}
                disabled={!newGroupName.trim()}
                className="bg-primary/10 text-primary p-2 rounded-lg hover:bg-primary/20 disabled:opacity-50"
              >
                <Plus size={18} />
              </button>
            </div>
            ...
            <div className="space-y-2 max-h-[150px] overflow-y-auto custom-scrollbar pr-1">
              {groups.length === 0 && (
                <p className="text-xs text-secondary/50 text-center py-2 italic">No groups created</p>
              )}
              {groups.map(group => (
                <div key={group.id} className="flex items-center justify-between bg-surfaceHighlight/30 p-2 rounded-lg border border-border/30 group">
                  <div className="flex items-center gap-2 overflow-hidden">
                    <span className={`text-sm truncate ${!group.enabled ? 'text-secondary line-through' : 'text-primary'}`}>
                      {group.name}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <div
                      ...
                      className={`
                        w-8 h-4 rounded-full cursor-pointer relative transition-all duration-200
                        ${group.enabled ? 'bg-primary' : 'bg-surfaceHighlight border border-border'}
                      `}
                    >
                      <div className={`
                        absolute top-0.5 left-0.5 w-3 h-3 rounded-full transition-all duration-200 bg-black
                        ${group.enabled ? 'translate-x-[16px]' : 'translate-x-0 bg-secondary/50'}
                      `} />
                    </div>
                    <button
          ...
          <div className={`bg-surface border ${editingId ? 'border-primary/50' : 'border-border'} rounded-xl backdrop-blur-sm bg-opacity-80 shadow-lg transition-all duration-300 overflow-hidden flex flex-col flex-1`}>
            {/* Header */}
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full bg-surfaceHighlight border border-border text-primary rounded-lg px-4 py-2.5 text-sm appearance-none focus:outline-none focus:border-primary disabled:opacity-50"
                                  ...
                                  if (group) {
                                    return (
                                      ...
                                      ${group.enabled
                                          ...
                                          : 'bg-surfaceHighlight text-secondary border-border line-through opacity-70'
                                        }
                                    `}>
                                        <Folder size={10} />
                                        {group.name}
