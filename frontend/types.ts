...
// --- UI Types (moved up to avoid forward reference issues) ---
...
export interface ScheduleItem {
  ...
  dayOffset?: 0 | 1;  // 0 = same day (morning), 1 = next day (night) - only for times 00:00-05:59
...
export interface ElectronAPI {
  ...
  saveSchedule: (data: { scheduler: ScheduleItem[], groups?: SchedulerGroup[] }) => Promise<{ success: boolean; error?: string }>;
  saveSchedulerGroups: (groups: SchedulerGroup[]) => Promise<{ success: boolean; error?: string }>;

  scheduleCreate: (job: any) => Promise<{ success: boolean; error?: string }>;
