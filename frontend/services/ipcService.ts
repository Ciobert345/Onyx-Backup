import { AppConfig, SystemStatus, JobStatus, ScheduleItem, SchedulerGroup, BackupConfig, Preferences, SystemStats, HistoryEvent } from '../types';
...
export const ipcService = {
  ...
  getSchedule: async (): Promise<{ scheduler: ScheduleItem[], groups: SchedulerGroup[] }> => {
    if (window.electronAPI) {
      const res = await window.electronAPI.getConfig();
      if (res.success && res.config) {
        return {
          scheduler: res.config.scheduler || [],
          groups: res.config.schedulerGroups || []
        };
      }
    }
  ...
  saveSchedulerGroups: async (groups: SchedulerGroup[]): Promise<boolean> => {
    if (window.electronAPI) {
      const res = await window.electronAPI.saveSchedulerGroups(groups);
      return res.success;
    }
    return false;
  },
  ...
  saveSchedule: async (data: { scheduler: ScheduleItem[], groups?: SchedulerGroup[] }): Promise<boolean> => {
    if (window.electronAPI) {
