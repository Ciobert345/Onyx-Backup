...
async function startSync(jobId, onProgress = null) {
    const jobs = configManager.get('jobs');
    const job = jobs.find(j => j.id === jobId);
    if (!job) {
    ...
    if (job.dest.startsWith('drive://') || job.source.startsWith('drive://')) {
        logger.info('Drive Sync requested. Delegating to BackupManager (One-way Sync).');
        try {
            const result = await backupManager.startBackup(jobId, null, (progress) => {
                globalProgress = progress;
                if (onProgress) onProgress(progress);
            });
            console.log('[DEBUG] syncManager received from backupManager:', JSON.stringify(result));
            return result;
        } finally {
            globalProgress = null;
        }
    ...
    try {
        if (onProgress) {
            // Wrap callback to update global state
            const originalOnProgress = onProgress;
            onProgress = (prog) => {
                globalProgress = prog;
                originalOnProgress(prog);
            };
        } else {
            onProgress = (prog) => {
                globalProgress = prog;
        ...
        onProgress({ processed: 0, total: 0, currentFile: 'Analyzing changes...' });

        const changes = await analyzeChanges(jobId);
        const totalOps = changes.copyToDest.length + changes.copyToSource.length;
        let processed = 0;

        onProgress({ processed: 0, total: totalOps, currentFile: `Found ${totalOps} changes...` });
        ...
        for (const relPath of changes.copyToDest) {
            ...
            onProgress({ processed: processed, total: totalOps, currentFile: `Syncing ${relPath} -> Dest` });
        ...
        for (const relPath of changes.copyToSource) {
            ...
            onProgress({ processed: processed, total: totalOps, currentFile: `Syncing ${relPath} -> Source` });
        ...
        // The allExclusions variable is already defined above, so we don't redefine it.
...
module.exports = { startSync, analyzeChanges, getProgress };
