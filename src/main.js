...
function registerIpcHandlers() {
        ...
        try {
            ...
            if (scheduleData.groups) {
                if (Array.isArray(scheduleData.groups)) {
                    configManager.set('schedulerGroups', scheduleData.groups);
                }
            }
    ...
    ipcMain.handle('saveSchedulerGroups', async (event, groups) => {
        try {
            if (!Array.isArray(groups)) {
                return { success: false, error: 'Groups must be an array' };
            }
            configManager.set('schedulerGroups', groups);
            scheduler.initScheduler();
            return { success: true };
        } catch (e) {
            logger.error(`Error saving scheduler groups: ${e.message}`);
            return { success: false, error: e.message };
        }
    });
...
function createWindow() {
    ...
    const windowHeight = Math.floor(screenHeight * 0.88);
