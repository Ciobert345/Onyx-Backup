const { app, ipcMain, BrowserWindow, dialog, shell, screen } = require('electron');
...
function registerIpcHandlers() {
    ...
    ipcMain.handle('startBackup', async (event, jobId) => {
        try {
            await backupManager.startBackup(jobId);
            return { success: true };
        } catch (e) {
    ...
    ipcMain.handle('startSync', async (event, jobId) => {
        try {
            await syncManager.startSync(jobId);
            return { success: true };
        } catch (e) {
            return { success: false, error: e.message };
        }
    ...
    ipcMain.handle('getStatus', (event) => {
        ...
        return {
            ...
            lastBackup: status.lastBackup ? new Date(status.lastBackup).toLocaleString() : 'Never',
